...
def build_llm_payload(model_dir: Path, model_id: str, query: dict[str, list[str]]) -> dict[str, Any]:
    ...
    nodes = [
        ...
        build_node(
            ...
            4,
            "Attention Output",
            "softmax(QK^T)V + output projection",
            "聚合 Q / K / V 后得到注意力输出，并投影回隐藏维度。",
            attention_shape,
            hidden_shape,
            attention_badges,
            ...
            [
                ...
                section(
                    "推导公式",
                    [
                        detail("attention weights", f"Q x K^T -> [B, heads, T, T] 后经 softmax"),
                        detail("value mix", f"softmax(QK^T) x V -> [B, T, n_heads, head_dim] = {attention_shape}"),
                        detail("output projection", f"concat heads -> [B, T, H] = {hidden_shape}"),
                    ],
                ),
            ],
            "core",
            micro_flow=["QK score", "Softmax", "Weighted V", "Output proj"],
            parent_id="decoder_stack",
            view_modes=["block"],
        ),
        build_node(
    ...
    edges = [
        ...
        build_edge("decoder_q_proj", "decoder_attn_output", "Q", ["block"]),
        build_edge("decoder_k_proj", "decoder_attn_output", "K", ["block"]),
        build_edge("decoder_v_proj", "decoder_attn_output", "V", ["block"]),
        build_edge("decoder_attn_output", "decoder_ffn", "attention output", ["block"]),
